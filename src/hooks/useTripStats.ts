...
export function useTripStats(trip: Trip, dayPlans: DayPlan[], spots: Spot[], expenses: Expense[] = []) {
  return computed(() => ({
    ...
    budget: budgetStatus(trip, dayPlans.filter((day) => day.trip_id === trip.id), spots, expenses),
  }));
}
